#!/bin/bash
# Airbnb Booking Import - alle 50 Buchungen
# Ausführen mit: bash airbnb-import.sh
...
import() {
  ...
  if [ $exit_code -ne 0 ]; then
    echo "FEHLER [$code] $name — curl exit $exit_code"
    errors=$((errors+1))
  elif echo "$resp" | grep -q '"duplicate":true'; then
    echo "DUPLIKAT [$code] $name"
    dupes=$((dupes+1))
  elif echo "$resp" | grep -q '"duplicate":false\|"id":\|"success"'; then
    echo "NEU     [$code] $name ($checkIn → $checkOut, ${persons}P)"
    new=$((new+1))
  else
    echo "UNBEKANNT [$code] $name — $resp"
    errors=$((errors+1))
  fi
}
...
echo "=========================================="
echo "E-Mails gefunden:  50"
echo "Neu importiert:    $new"
echo "Duplikate:         $dupes"
echo "Fehler:            $errors"
echo "Gesamt verarbeitet: $total"
